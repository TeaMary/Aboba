...
const Login = () => {
	...
	const auth = (username, password) => {
		(!username || !password) ?
			setAuthError(true) : (setAuthError(false));
		...
		fetch("http://tl.h30.ru/api/login",
			...
			.then((data) =>
				(data.status === 200) ?
					setAuthorized(true) : setAuthError(true)
			)
			.catch((e) => (e.status === 403) && setAuthError(true));
	}
			...
			{authorized ? (<div>Вы авторизованны</div>) : (
				<div className='login__screen'>
					...
					<div className='login__box'>
						<div className="login_error">{authError && ('Вы ввели неправильный логин или пароль')}</div>
